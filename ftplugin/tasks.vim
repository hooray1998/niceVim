...
call s:initVariable('g:TasksDateFormat', '%m-%d %H:%M')
...
let s:regDone = g:TasksAttributeMarker . 'D'
let s:regCancelled = g:TasksAttributeMarker . 'C'
...
function! GetProjects()
  ...
  let l:lineNr = line('.') - 1
  let l:lastSpace = match(getline("'."), "\\S")
  let l:results = []
  while l:lineNr > 0
    let l:match = matchstr(getline(l:lineNr), s:regProject)
    let l:curSpace = match(getline(l:lineNr), "\\S")
    if len(l:match) && l:curSpace < l:lastSpace
      let l:lastSpace = l:curSpace
...
function! TaskComplete()
  ...
  if l:isMatch > -1
    if l:doneMatch > -1
      ...
      call AddAttribute('D', strftime(s:dateFormat))
      call AddAttribute('P', join(l:projects, ' / '))
    endif
  endif
endfunc
...
function! TaskCancel()
  ...
  if l:isMatch > -1
    if l:cancelledMatch > -1
      ...
      call AddAttribute('C', strftime(s:dateFormat))
      call AddAttribute('P', join(l:projects, ' / '))
    endif
  endif
endfunc
...
function! TasksArchive()
  ...
  if l:archiveLine == -1
    ...
    exec '%s#\($\n\s*\)\+\%$##'
    exec 'normal Go'
    exec 'normal `' . g:TasksArchiveSeparator
    exec 'normal `Archive:'
